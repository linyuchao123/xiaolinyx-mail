...
import outboundQuotaService from './outbound-quota-service';
...
const emailService = {
	...
	async send(c, params, userId) {
		...
		if (!Array.isArray(receiveEmail) || receiveEmail.length < 1 ||
			receiveEmail.length > 10 || !receiveEmail.every(value =>
				typeof value === 'string' && verifyUtils.isEmail(value))) {
			throw new BizError(t('invalidRecipients'), 400);
		}
		...
		if (c.env.admin !== userRow.email && roleRow.sendCount) {
			...
			if (Number(userRow.sendCount) + receiveEmail.length > roleRow.sendCount) {
		...
		if (c.env.admin !== userRow.email && roleRow.sendCount && roleRow.sendType !== 'internal') {
			reservedQuota = await userService.reserveSendCount(
				c, receiveEmail.length, userId, roleRow.sendCount
			);
			if (!reservedQuota) {
				throw new BizError(t(roleRow.sendType === 'day' ? 'daySendLimit' : 'totalSendLimit'), 403);
			}
		}

		let reservedGlobalQuota = false;
		let sendResult = {};
		try {
			if (!allInternal && !useCloudflareEmail) {
				reservedGlobalQuota = await outboundQuotaService.reserve(c, receiveEmail.length);
				if (!reservedGlobalQuota) throw new BizError(t('dailyProviderLimit'), 429);
			}
		...
		if (sendResult.error) {
			...
			if (reservedGlobalQuota) {
				await outboundQuotaService.refund(c, receiveEmail.length);
			}
			if (reservedQuota) {
				await userService.refundSendCount(c, receiveEmail.length, userId);
			}
			throw error;
		}
