...
const settingService = {
	...
	async set(c, params) {
		...
		const nextRegister = params.register === undefined ? settingData.register : Number(params.register);
		if (nextRegister === 0) {
			const nextVerify = params.registerVerify === undefined
				? settingData.registerVerify : Number(params.registerVerify);
			const siteKey = params.siteKey === undefined ? settingData.siteKey : params.siteKey;
			const secretKey = params.secretKey === undefined ? settingData.secretKey : params.secretKey;
			if (nextVerify !== 0 || !siteKey || !secretKey) {
				throw new BizError(t('publicRegistrationRequiresTurnstile'), 403);
			}
		}
		const r2Domain = params.r2Domain === undefined ? settingData.r2Domain : params.r2Domain;
		if (typeof r2Domain === 'string' && r2Domain.trim()) {
			throw new BizError(t('publicAttachmentDomainDisabled'), 403);
		}
