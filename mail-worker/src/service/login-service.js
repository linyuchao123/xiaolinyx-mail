...
const loginService = {
	...
	async register(c, params, oauth = false) {
		...
		let { regKey, register, registerVerify, regVerifyCount, minEmailPrefix,
			emailPrefixFilter, siteKey, secretKey } = await settingService.query(c)

		if (oauth) {
			registerVerify = settingConst.registerVerify.CLOSE;
		}

		const isAdmin = email === c.env.admin?.toLowerCase();
		const isBootstrap = isAdmin && c.env.jwt_secret &&
			c.req.header('X-Init-Secret') === c.env.jwt_secret;

		if (isAdmin && !isBootstrap) {
			throw new BizError(t('adminBootstrapRequired'), 403);
		}

		if (register === settingConst.register.CLOSE && !isBootstrap) {
			throw new BizError(t('regDisabled'));
		}

		if (!isBootstrap && !oauth &&
			(registerVerify !== settingConst.registerVerify.OPEN || !siteKey || !secretKey)) {
			throw new BizError(t('publicRegistrationRequiresTurnstile'), 403);
		}
		...
		if (!isBootstrap && registerVerify === settingConst.registerVerify.OPEN) {
			regVerifyOpen = true
			await turnstileService.verify(c,token)
		}

		if (!isBootstrap && registerVerify === settingConst.registerVerify.COUNT) {
