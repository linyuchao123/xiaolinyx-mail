...
const dbInit = {
	async init(c) {
		...
		if (!secret || !c.env.jwt_secret || secret !== c.env.jwt_secret) {
			return c.text('❌ Initialization secret mismatch', 401);
		}
    ...
    `).run();
        ...
        INSERT INTO role (
          ...
          1, '普通用户', NULL, '0000-00-00 00:00:00', 0, '每日最多发送 10 封邮件', 0, 1, 10, 'day', 3
        )
      `).run();
		}
